...
export default function Appearance() {
  ...
  // Fetch config whenever the selected bot changes
  useEffect(() => {
    if (!selectedBotId) return;
    setLoadingConfig(true);
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
    fetch(`${apiUrl}/api/config?bot_id=${encodeURIComponent(selectedBotId)}`)
      .then(res => res.json())
      .then(data => {
          ...
          <div className="lg:col-span-2">
            <form onSubmit={handleSave} className="bg-white rounded-xl border border-gray-200 p-6 space-y-6">
              ...
              <div className="pt-4 border-t border-gray-100 flex justify-end">
                <button type="submit" disabled={saving || !selectedBotId}
